...
namespace iDeal
{
    public class iDealService
    {
        ...
        public iDealService(string merchantId) : this(new DefaultConfiguration((ConfigurationSectionHandler)ConfigurationManager.GetSection("iDeal"), merchantId)) { }
        ...
        /// </summary>
        public DirectoryResponse SendDirectoryRequest(ref iDealException exception)
        {
            ...
            return (DirectoryResponse)_iDealHttpRequest.SendRequest(directoryRequest, _signatureProvider, _configuration.AcquirerUrl, _iDealHttpResponseHandler, ref exception);
        }
        ...
        public TransactionResponse SendTransactionRequest(string issuerCode, string merchantReturnUrl, string purchaseId, decimal amount, TimeSpan? expirationPeriod, string description, string entranceCode, ref iDealException exception)
        {
            // Set up dependencies for http request
            var transactionRequest = new TransactionRequest(_configuration.MerchantId, _configuration.MerchantSubId, issuerCode, merchantReturnUrl, purchaseId, amount, expirationPeriod, description, entranceCode);

            // Execute http request
            return (TransactionResponse)_iDealHttpRequest.SendRequest(transactionRequest, _signatureProvider, _configuration.AcquirerUrl, _iDealHttpResponseHandler, ref exception);
        }
        ...
        public StatusResponse SendStatusRequest(string transactionId, ref iDealException exception)
        {
            // Set up dependencies for http request
            var statusRequest = new StatusRequest(_configuration.MerchantId, _configuration.MerchantSubId, transactionId);

            // Execute http request
            var result = _iDealHttpRequest.SendRequest(statusRequest, _signatureProvider, _configuration.AcquirerUrl, _iDealHttpResponseHandler, ref exception);
            if (result != null)
            {
